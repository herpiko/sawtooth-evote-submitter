...
const schema = {
  properties: {
    ...
    verb : {
      message : 'verb [registered, ready, invalid, death, vote]',
      required : true,
      default : 'registered'
    },
    vote : {
...
const familyName = 'dpt';

const name = createHash('sha256').update(result.voterId).digest('hex');
console.log(name);
if (result.verb === 'vote' && !result.vote) {
  console.log('Vote should not be empty');
  process.exit(1);
}

if (result.verb != 'vote') {
  result.vote = result.verb;
}
const payload = {
    Verb: result.verb,
    Name: name,
